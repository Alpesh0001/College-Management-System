...
import com.example.collegemanagementsystemfaculty.SplashActivity
...
open class CoreBaseActivity : AppCompatActivity() {
    ...
    private fun attachOverlayOnTop() {
        ...
        val existing = root.findViewById<View>(
            com.example.collegemanagementsystemfaculty.R.id.blockingOverlay
        )
        if (existing != null) {
            overlay = existing
            loaderView = existing.findViewById(
                com.example.collegemanagementsystemfaculty.R.id.imgLoader
            )
            statusText = existing.findViewById(
                com.example.collegemanagementsystemfaculty.R.id.tvStatus
            )
            progressRing = existing.findViewById(
                com.example.collegemanagementsystemfaculty.R.id.progressRing
            )
        ...
        val overlayView = LayoutInflater.from(this)
            .inflate(
                com.example.collegemanagementsystemfaculty.R.layout.view_blocking_loader,
                root,
                false
            )
        ...
        progressRing = overlayView.findViewById(
            com.example.collegemanagementsystemfaculty.R.id.progressRing
        )
        overlay?.visibility = View.GONE
    ...
    // ═══════════════════════════════════════════════════════════════
    // ✅ SIMPLE LOADING METHODS - USE IN ALL ACTIVITIES
    // ═══════════════════════════════════════════════════════════════

    /**
     * Show full screen blocking loader with custom message
     * @param message Loading message (e.g. "Verifying...", "Logging in...")
     */
    ...
    /**
     * Show simple progress ring (no message)
     */
    ...
    private fun startLoaderAnimation() {
        ...
        val anim = AnimationUtils.loadAnimation(
            this,
            com.example.collegemanagementsystemfaculty.R.anim.progress_ring_rotate
        )
    ...
    private fun showDialog() {
        ...
        val dialogView = layoutInflater.inflate(
            com.example.collegemanagementsystemfaculty.R.layout.dialog_no_internet,
            null
        )
        ...
        val tvTryAgain =
            dialogView.findViewById<View>(
                com.example.collegemanagementsystemfaculty.R.id.tvTryAgain
            )
